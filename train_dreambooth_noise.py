...
class MVTecDataset(Dataset):
    """
    DreamBooth 미세 조정을 위해 Instance(결함) 및 Class(정상) 이미지와 프롬프트를 준비하는 데이터셋입니다.
    이미지를 전처리하고 프롬프트를 토큰화합니다.
    """
    ...
    def __init__(
        ...
        size=512,
        center_crop=False,
        num_images=None, # 👈 참고: 이 인자는 현재 코드에서 무시되고 `// 3` 로직으로 덮어써집니다.
    ):
        self.size = size
        self.center_crop = center_crop
        self.tokenizer = tokenizer

        # 1. Instance (결함) 이미지 및 마스크 경로 설정
        self.instance_data_root = Path(instance_data_root)
        if not self.instance_data_root.exists():
            raise ValueError("Instance images root doesn't exist.")
        ...
        # 헬퍼 함수: 파일 이름(e.g., '001.png')에서 숫자를 추출하여 정렬 기준으로 사용
        def extract_last_number(path):
            stem = path.stem  # 파일 확장자 제외 (e.g., '001_mask' or '001')
            numbers = re.findall(r"\d+", stem) # 모든 숫자 찾기
            return int(numbers[-1]) if numbers else -1 # 마지막 숫자를 int로 반환

        # 이미지와 마스크 경로를 숫자 순서대로 정렬 (e.g., 001.png <-> 001_mask.png 짝 맞추기)
        self.instance_images_path = sorted(self.instance_data_root.iterdir(), key=extract_last_number)
        self.mask_images_path = sorted(self.mask_data_root.iterdir(), key=extract_last_number)

        # 2. ❗️ "Few-Shot" 환경을 위한 데이터 수량 강제 조절 (핵심 로직) ❗️
        # if self.instance_images_path:
        #     # 발견된 총 결함 이미지의 1/3만 사용하도록 강제 (논문 저자의 실험 설정)
        #     num_images = len(self.instance_images_path) // 3
            
        #     # 이미지와 마스크 리스트를 1/3 크기로 자름
        #     self.instance_images_path = self.instance_images_path[:num_images]
        #     self.mask_images_path = self.mask_images_path[:num_images]

        # 실제로 사용할 결함 이미지 개수를 저장
        self.num_instance_images = len(self.instance_images_path)
        self.instance_prompt = instance_prompt
        
        # 데이터셋의 기본 길이를 결함 이미지 수로 설정
        self._length = self.num_instance_images

        # 3. Class (정상 / Prior Preservation) 이미지 처리
        if class_data_root is not None:
            self.class_data_root = Path(class_data_root)
            if not self.class_data_root.exists():
                raise ValueError("Class images root doesn't exist.")
            
            # 정상 이미지 경로도 숫자 순으로 정렬
            self.class_images_path = sorted(self.class_data_root.iterdir(), key=extract_last_number)
            
            if num_images is not None: # (위의 '// 3' 로직으로 인해 num_images는 1/3 값임)
                # 만약 정상 이미지 수가 1/3 값보다 적으면 에러
                if num_images > len(self.class_images_path):
                    raise ValueError(
                        f"Requested number of class images ({num_images}) exceeds the available number of images ({len(self.class_images_path)})."
                    )
                
                # ❗️ 정상 이미지도 결함 이미지와 동일한 개수 (1/3) 만큼만 사용하도록 자름
                self.class_images_path = self.class_images_path[:num_images]
            
            self.num_class_images = len(self.class_images_path)
            
            # 데이터셋의 최종 길이를 instance와 class 중 더 큰 값으로 설정
            # (이 코드에서는 두 값이 동일하게 맞춰짐)
            self._length = max(self.num_class_images, self.num_instance_images)
            self.class_prompt = class_prompt
        else:
            self.class_data_root = None
        ...
        # 2단계: 텐서 변환 및 정규화 (PIL 이미지 -> Tensor)
        self.image_transforms = transforms.Compose(
            [
    ...
    def __len__(self):
        # 데이터셋의 총 길이(epoch당 스텝 수)를 반환
        return self._length

    def __getitem__(self, index):
        # 데이터 로더가 index에 해당하는 데이터를 요청할 때 호출됨
        example = {}

        # --- Instance(결함) 데이터 로딩 ---
        # `%` 연산자: index가 데이터셋 길이를 초과해도 순환하며 이미지를 가져옴 (e.g., 4 % 4 -> 0)
        instance_image = Image.open(self.instance_images_path[index % self.num_instance_images])
        if instance_image.mode != "RGB":
            instance_image = instance_image.convert("RGB") # 3채널 RGB 보장
        ...
        mask_image = Image.open(self.mask_images_path[index % self.num_instance_images])
        if mask_image.mode != "L":
            mask_image = mask_image.convert("L") # 1채널 흑백(L) 보장
        
        # 1단계 전처리 (리사이즈/크롭)
        mask_image = self.image_transforms_resize_and_crop(mask_image)

        # 2단계 전처리 (텐서 변환 및 정규화)
        example["mask_PIL_images"] = mask_image # (전처리 전 PIL 이미지 - 아마도 디버깅/시각화용)
        example["PIL_images"] = instance_image # (전처리 전 PIL 이미지 - 아마도 디버깅/시각화용)
        example["instance_images"] = self.image_transforms(instance_image)
        example["mask_images"] = self.image_transforms(mask_image)

        # Instance(결함) 프롬프트 토큰화 (e.g., "a photo of sks defect")
        example["instance_prompt_ids"] = self.tokenizer(
            self.instance_prompt,
            padding="do_not_pad",
            truncation=True,
        ...
        # --- Class(정상) 데이터 로딩 (Prior Preservation용) ---
        if self.class_data_root:
            class_image = Image.open(self.class_images_path[index % self.num_class_images])
            if class_image.mode != "RGB":
                class_image = class_image.convert("RGB")
            ...
            example["class_images"] = self.image_transforms(class_image)
            example["class_PIL_images"] = class_image # (전처리 전 PIL 이미지)
            
            # Class(정상) 프롬프트 토큰화 (e.g., "a photo of defect")
            example["class_prompt_ids"] = self.tokenizer(
                self.class_prompt,
                padding="do_not_pad",
                truncation=True,
        ...
        # 최종적으로 { instance 데이터, class 데이터 }가 담긴 딕셔너리를 반환
        return example
